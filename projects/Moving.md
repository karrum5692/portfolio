...
> 실사용 규모 데이터 약 685만 행 생성 (견적 169만 · 이력 122만 · 채팅 61만 · 견적요청 55만 · 리뷰 37만 · 회원 11만) 데이터에서 관리자 대시보드 API를 33.8초에서 0.38초로 개선하고, k6 부하 테스트로 300 VU까지 검증했습니다. 그 과정에서 같은 도메인이 두 번 병목이 되었고, 두 번의 원인은 서로 달랐습니다.
